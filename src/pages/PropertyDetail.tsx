...
export function PropertyDetail() {
  ...
  if (isLoading) return <><Topbar title="Ακίνητο" /><DetailPageSkeleton /></>
  if (!property) return <div className="p-8 text-slate-400 dark:text-slate-500">Δεν βρέθηκε το ακίνητο.</div>
    ...
    <div>
      ...
      <div className="p-4 lg:p-6 space-y-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">{property.address}</h2>
                <Badge label={PROPERTY_STATUS_LABELS[property.status]} variant={property.status} />
              </div>
              <p className="text-slate-500 dark:text-slate-400">{[property.neighborhood, property.city, property.postal_code].filter(Boolean).join(', ')}</p>
            </div>
            <div className="text-right">
              <p className="text-3xl font-bold text-slate-900 dark:text-white">{fmtMoney(property.list_price)}</p>
              {property.list_price && property.sqm && <p className="text-sm text-slate-400 dark:text-slate-500">{pricePerSqm(property.list_price, property.sqm)}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm p-5">
            <h3 className="font-semibold text-slate-900 dark:text-white mb-3">Στοιχεία Ακινήτου</h3>
            ...
            {property.description && <p className="text-sm text-slate-500 dark:text-slate-400 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700">{property.description}</p>}
          </div>

          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700">
                <h3 className="font-semibold text-slate-900 dark:text-white">Προσφορές ({offers.length})</h3>
                <div className="flex gap-2">
                  {offers.length >= 2 && (
              ...
              {offers.length === 0
                ? <div className="p-8 text-center text-slate-400 dark:text-slate-500 text-sm">Δεν υπάρχουν προσφορές για αυτό το ακίνητο.</div>
