...
import { Menu, Search, Building2, FileText, Users } from 'lucide-react'
...
function GlobalSearch() {
  ...
  return (
    <div ref={ref} className="relative hidden sm:block">
      <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-3 py-1.5 w-56 focus-within:ring-2 focus-within:ring-blue-500 focus-within:bg-white transition-all">
      ...
      {open && results.length > 0 && (
        <div className="absolute top-full mt-1 left-0 w-72 bg-white rounded-xl shadow-lg border border-slate-200 z-50 overflow-hidden">
          {results.map((r, i) => {
      ...
      {open && q.length >= 2 && results.length === 0 && (
        <div className="absolute top-full mt-1 left-0 w-64 bg-white rounded-xl shadow-lg border border-slate-200 z-50 px-4 py-3 text-sm text-slate-400">
...
export function Topbar({ title, actions }: TopbarProps) {
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-4 lg:px-6 py-3 flex items-center justify-between gap-4 sticky top-0 z-20">
      <div className="flex items-center gap-3 min-w-0">
        <button onClick={toggleSidebar} className="lg:hidden p-1.5 rounded-lg hover:bg-slate-100 shrink-0">
          <Menu size={20} />
        </button>
